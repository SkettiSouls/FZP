...
fn mode_directory() {
    ...
    match cli {
        Cli { directory: Some(args), .. } => {
            ...
            // Debug tool, REMOVE
            println!("DIRECTORY DIR: {music_directory:#?}");
            ...
            // list directories in fuzzy finder
                // logic to remove playlist files
            // play selected directories
            // exit logic
        },
...
fn mode_fuzzy() {
    ...
    match cli {
        Cli { path: Some(music_directory), .. } => {
            // DEBUG
            println!("FUZZY DIR: {music_directory:#?}");
...
fn mode_playlist() {
    ...
    match cli {
        Cli { playlist: Some(args), .. } => {
            ...
            // DEBUG
            println!("PLAYLIST DIR: {music_directory:#?}");

            set_current_dir(&music_directory).expect("Failed to change directory.");

            ///// pseudo code starts here: /////
            // startup audio service
            // list playlist files (m3u) in fuzzy finder
            // play selected playlist(s)
            // exit logic
        },
